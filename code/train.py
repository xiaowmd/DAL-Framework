...
def train_model(
    ...
    # 定义损失函数和优化器
    criterion = torch.nn.NLLLoss()  # 分类问题使用 NLLLoss
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            best_val_report = val_report  # 保存当前最佳报告
            model_save_path = os.path.join(
                save_dir, f"best_model_{label_num}.pth"
            )
            try:
    ...
    # 输出验证结果
    print("\nValidation Results:")
    f1_score = best_val_report["macro avg"]["f1-score"]
    precision = best_val_report["macro avg"]["precision"]
    recall = best_val_report["macro avg"]["recall"]
    cover_accuracy = best_val_report["cover_metrics"]["accuracy"]
    cover_f1_score = best_val_report["cover_metrics"]["f1-score"]
    cover_precision = best_val_report["cover_metrics"]["precision"]
    cover_recall = best_val_report["cover_metrics"]["recall"]
    cover_threshold = best_val_report["cover_threshold"]
    print(
        f"Best Val Accuracy: {best_val_acc:.4f}, F1 Score: {f1_score:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}"
    )
    print(
    ...
    if save_results:
        results_df.to_excel(
            os.path.join(
                results_save_path, f"validation_results_{label_num}.xlsx"
            ),
            index=False,
        )
        print(f"Results saved to {results_save_path}/validation_results_{label_num}.xlsx")
...
def train_with_cross_validation(
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        # 定义损失函数和优化器
        criterion = torch.nn.NLLLoss()  # 分类问题使用 NLLLoss
        # optimizer = optim.Adam(model.parameters(), lr=0.001, alpha=weight_decay, weight_decay=l2_penalty)
        optimizer = optim.Adam(model.parameters(), lr=0.001)
...
if __name__ == "__main__":
    embedding_folder = "Embeddings_cls_hgd"  # 嵌入文件路径
    label_file = "labels/16.txt"  # 标签文件路径
    label_num = 16  # 标签对应的问题编号
    ...
    results_save_path = "Results_of_GBAN"  # 结果保存路径 Data_size_test用于确定测试集大小，DL_results用于保存结果,DL_results_GBN     # 确定数据集规模实验：80，160,240,320，400
    save_results = True  # 是否保存结果
    random_search = False  # 是否进行随机搜索超参数组合搜索
    save_dir = "./saved_models_GBAN"  # 模型保存目录
    ...
    if cross_vali:
        ...
        if All_questions:
            ...
            label_file = "labels/" + str(label_num) + ".txt"
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            print(f"Label file: {label_file}")
